...
function replaceAll(f, from, to) {
    if (fs.existsSync(f)) {
        fs.writeFileSync(f, fs.readFileSync(f, 'utf8').replace(from, to), 'utf8');
    }
}

// src/constants.ts
const content = `import { Collection, ArchiveCard, Source, Message, Artifact, View } from './types';
...
let sm = fs.readFileSync('server/models/sourceModel.js', 'utf8');
sm = sm.replace(/鏈厤缃\uFFFD\?\);/g, "鏈厤缃');"); // handle any leftover
sm = sm.replace(/\uFFFD\?\);/g, "');");
fs.writeFileSync('server/models/sourceModel.js', sm, 'utf8');

// server/models/artifactModel.js, etc just to be sure
['server/models/artifactModel.js', 'server/models/collectionModel.js', 'server/models/messageModel.js', 'server/models/galleryModel.js'].forEach(f => {
